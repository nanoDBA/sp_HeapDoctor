...
BEGIN
    RAISERROR(N'  PASS: 9V - Version starts with 1.x.2026.x.', 10, 1) WITH NOWAIT;
    UPDATE #TestCounts SET PassCount += 1;
END
ELSE
BEGIN
    ...
    DECLARE @Msg nvarchar(4000); SET @Msg =N'  FAIL: 9V - Expected version 1.x.2026.x, got: ' + ISNULL(@v9, N'NULL');
    RAISERROR(@Msg, 10, 1) WITH NOWAIT;
    UPDATE #TestCounts SET FailCount += 1;
END
GO
