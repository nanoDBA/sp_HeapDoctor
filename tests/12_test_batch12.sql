...
Tests:
  -- Smoke Tests --
  12A - Version matches dbo.ExpectedVersion
...
IF @12a_version = (SELECT version FROM dbo.ExpectedVersion)
    RAISERROR(N'  PASS 12A: Version matches dbo.ExpectedVersion.', 10, 1) WITH NOWAIT;
ELSE
BEGIN
    DECLARE @12a_msg nvarchar(200) = N'  FAIL 12A: Expected ' + (SELECT version FROM dbo.ExpectedVersion) + N', got ' + ISNULL(@12a_version, N'NULL');
