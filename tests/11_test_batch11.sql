...
------------------------------------------------------------------------
-- 11B: Smoke test - Version is 2026.07.31.3
------------------------------------------------------------------------
RAISERROR(N'--- 11B: Version check ---', 10, 1) WITH NOWAIT;

IF EXISTS (SELECT 1 FROM #Results WHERE version = N'2026.07.31.3')
BEGIN
    RAISERROR(N'  PASS 11B: Version is 2026.07.31.3.', 10, 1) WITH NOWAIT;
    UPDATE #TestCounts SET PassCount += 1;
END
ELSE
BEGIN
    ...
    DECLARE @ver_msg nvarchar(200) = N'  FAIL 11B: Expected version 2026.07.31.3, got ' + ISNULL(@actual_ver, N'NULL');
