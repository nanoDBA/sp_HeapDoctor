...
------------------------------------------------------------------------
-- 11B: Smoke test - Version is 1.0.2026.0302e
------------------------------------------------------------------------
RAISERROR(N'--- 11B: Version check ---', 10, 1) WITH NOWAIT;

IF EXISTS (SELECT 1 FROM #Results WHERE version = N'1.0.2026.0302e')
BEGIN
    RAISERROR(N'  PASS 11B: Version is 1.0.2026.0302e.', 10, 1) WITH NOWAIT;
    UPDATE #TestCounts SET PassCount += 1;
END
ELSE
BEGIN
    ...
    DECLARE @ver_msg nvarchar(200) = N'  FAIL 11B: Expected version 1.0.2026.0302e, got ' + ISNULL(@actual_ver, N'NULL');
