...
Tests Batch 11 additions:
  ...
  11B - Version matches dbo.ExpectedVersion
...
------------------------------------------------------------------------
-- 11B: Smoke test - Version matches dbo.ExpectedVersion
------------------------------------------------------------------------
RAISERROR(N'--- 11B: Version check ---', 10, 1) WITH NOWAIT;

IF EXISTS (SELECT 1 FROM #Results r JOIN dbo.ExpectedVersion e ON r.version = e.version)
BEGIN
    RAISERROR(N'  PASS 11B: Version matches dbo.ExpectedVersion.', 10, 1) WITH NOWAIT;
    UPDATE #TestCounts SET PassCount += 1;
END
ELSE
BEGIN
    ...
    DECLARE @ver_msg nvarchar(200) = N'  FAIL 11B: Expected version '
        + (SELECT version FROM dbo.ExpectedVersion) + N', got ' + ISNULL(@actual_ver, N'NULL');
...
RAISERROR(N'', 10, 1) WITH NOWAIT;
RAISERROR(N'========================================================================', 10, 1) WITH NOWAIT;
RAISERROR(N' Batch 11 Results', 10, 1) WITH NOWAIT;
RAISERROR(N'========================================================================', 10, 1) WITH NOWAIT;
